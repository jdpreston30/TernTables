...
| `` >_4_Positive_Nodes `` | More than 4 positive lymph nodes — n (%) |
...
columns like Male/Female — are shown with each level as an indented sub-row.
...
writexl::write_xlsx(tbl_descriptive, "Tern_descriptive.xlsx")
```
...
variables) and a `P` column (test *P* value for each variable).

Two-group comparison table exported to Word, with odds ratios and category section headers:

![Two-group comparison exported to Word.](figures/tern_2_group.png){width=100%}
...
![Three-group comparison exported to Word.](figures/tern_3_group.png){width=100%}
