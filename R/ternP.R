...
ternP <- function(data) {
  ...
  for (col_nm in char_cols) {
    x       <- data[[col_nm]]
    lowered <- stringr::str_to_lower(x)
    if (dplyr::n_distinct(lowered, na.rm = TRUE) < dplyr::n_distinct(x, na.rm = TRUE)) {
      ...
      data[[col_nm]] <- stringr::str_to_title(x)
    }
  }
